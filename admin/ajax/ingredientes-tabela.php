...
if(in_array('gerenciar_composicao', $permissao)){
    ...
    foreach($itens as &$item){
        ...
        echo "<tr name='resultado' id='status".$item->getPkId()."'>
                ...
                <td style='text-align: center;' name='editar'><a href='ingrediente-view.php?cod=".$item->getPkId()."' style='font-size: 20px;' href='#'><button class='btn btn-kionux'><i class='fa fa-edit'></i>Editar</button></a></td>
                <td style='text-align: center;' name='status'  ><button type='button' onclick='removeIngrediente(".$item->getPkId().")' class='btn btn-kionux'><i class='fa fa-remove'></i>Excluir</button></td>
                </tr>";
    }
